...
const getBlockStyle = (currentStyle, appliedBlockStyles, data, text = '') => {
  ...
  if (currentStyle === 'atomic:image') {
    if (data && data.src) {
      return `![${text.trim()}](${data.src})`
    }
  }
...
function draftjsToMd(raw, extraMarkdownDict) {
  ...
  return raw.blocks
    .map(block => {
      ...
      // add block style
      returnString += getBlockStyle(block.type, appliedBlockStyles, block.data, block.text);
      appliedBlockStyles.push(block.type);

      const appliedStyles = [];
      // does not concat block text for image
